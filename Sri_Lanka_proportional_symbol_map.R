####################################################################################
### 
# R Code for visualization of proportional symbol map of SARS-CoV-2
...
map_2 <- ggplot() +geom_polygon(data = LK, aes(x=long, y = lat, group = group), fill="gray", 
  ...
  geom_point(data=uk_data, aes(x=long, y=lat, color = Location, size = Tests),
             alpha=.8, show.legend = T) + scale_size_continuous(range=c(1,14)) +
  theme(panel.border = element_rect(colour = "black", fill=F))+
  geom_text_repel(data=uk_data, aes(x=long, y=lat, label=loc), size=3.2, 
                  box.padding = .45, segment.colour = NA, max.overlaps = 50)+
...
### Save output
ggsave("SL373_prop_map.pdf",map , width=17, height=10, limitsize = T)
